...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_ddim2(self, x, t, t_next, Trans,clip_denoised=True, repeat_noise=False, condition_x=None, style=None):
        b, *_, device = *x.shape, x.device
        bt = extract(self.betas, t, x.shape)
        at = extract((1.0 - self.betas).cumprod(dim=0), t, x.shape)

        if condition_x is not None:
            et = self.denoise_fn(torch.cat([condition_x, x], dim=1), t,torch.cat((Trans,Trans) , dim=1))
        else:
    ...
    def p_sample_loop(self, x_in, continous=False, cand=None):
        ...
        if not self.conditional:
            ...
            x = x_in['SR']
            shape = x.shape
            b = shape[0]  # 2，3，256，256
            img = torch.randn(shape, device=device, generator=g_gpu)  # 2，3，256，256
            ret_img = x  # 2，3，256，256
            ...
            if self.sample_proc == 'ddpm':
                ...
                for j, i in enumerate(time_steps):
                    ...
                    if j == len(time_steps) - 1:
                        ...
                        t_next = torch.full((b,), time_steps[j + 1], device=device, dtype=torch.long)
                    img = self.p_sample_ddim2(img, t, t_next, x_in['Trans'],condition_x=x, style=x_in['style'])
                    if i % sample_inter == 0:
                        ret_img = torch.cat([ret_img, img], dim=0)
        if continous:
            return ret_img
    ...
    def q_sample_recover(self, x_noisy, t, predict_noise=None):
        return (x_noisy - extract(self.sqrt_one_minus_alphas_cumprod,
                                  t, x_noisy.shape) * predict_noise) / extract(self.sqrt_alphas_cumprod, t,
                                                                               x_noisy.shape)
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(
                torch.cat([condition_x, x_noisy], dim=1),t,torch.cat([x_in['Trans'], x_in['Trans']], dim=1))
