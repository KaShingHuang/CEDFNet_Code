...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        if modulator:#同一个layer的dim是一样的，不同层的dim不一样，相当于通道数，winsize始终没变为8，其实相当于对于一个图的每一个窗口加上一个随机噪声，即每个像素点的
            #每个通道都加上一个值
            self.modulator = nn.Embedding(win_size*win_size, dim) # modulator
        else:
            self.modulator = None

        if cross_modulator:   #都是没有这个东西的
        ...
        self.attn = WindowAttention(    #在窗口内计算窗口注意力
    ...
    def forward(self, x, mask=None):
        ...
        ## shift mask，这里是swin transformer原文中的shifted之后恢复原来位置要进行的统一化的masked计算
        ...
        if self.cross_modulator is not None:  #一直都是none不用理
            shortcut = x
        ...
        if self.modulator is not None:
            ...
            #if(self.modulator.weight.shape[1]==512): np.save('/data2/CarnegieBin_data/Kashing/tensor.npy', self.modulator.weight.cpu().detach().numpy())
        else:
            wmsa_in = x_windows
...
class BasicUformerLayer(nn.Module):   #一个BasicUformerLayer由多个LeWinTransformerBlock组成
    ...
    # build blocks，这里的代码默认都是shitf_flag=True的,具体在shift_size会有区别，所有阶段的cross_modulator都是False，编码的modulator是false解码是true
        if shift_flag:
            self.blocks = nn.ModuleList([
                LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=win_size,
                                    shift_size=0 if (i % 2 == 0) else win_size // 2,   #根据swin transformer的文章，几个连续的层是shifted和不进行shifed之间交互的
                                    mlp_ratio=mlp_ratio,
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,                          #img_size=128
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,    #depth变为[1, 2, 8, 8, 2, 8, 8, 2, 1]
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, token_projection='linear', token_mlp='leff',
                 dowsample=Downsample, upsample=Upsample, shift_flag=True, modulator=False, #这里的modulator为True
                 cross_modulator=False, **kwargs):
        super().__init__()

        self.num_enc_layers = len(depths)//2    #4个编码器和解码器，还有一个瓶颈阶段(不知道要来干什么的)
        ...
        # stochastic depth，编码器解码器和瓶颈阶段的随机dropout率
        ...
        # Input/Output，利用卷积层对输入的数据和输出的数据进行投影
...
if __name__ == "__main__":
    input_size = 256     #这里每一个patch都是256*256的
    arch = Uformer
