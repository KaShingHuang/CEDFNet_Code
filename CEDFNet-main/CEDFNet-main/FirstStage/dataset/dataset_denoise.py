...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        ...
        self.clean_images, self.noisy_images = [], []
        for clean_file, noisy_file in zip(clean_files, noisy_files):
            clean_path = os.path.join(rgb_dir, gt_dir, clean_file)
            noisy_path = os.path.join(rgb_dir, input_dir, noisy_file)
    ...
    def __getitem__(self, index):   #这个东西在  for data in train_loader的时候会被默认调用
        tar_index   = index
        #clean = torch.from_numpy(np.float32(load_img(self.clean_filenames[tar_index])))
        #noisy = torch.from_numpy(np.float32(load_img(self.noisy_filenames[tar_index])))

        clean = torch.from_numpy(np.float32(self.clean_images[tar_index]))
        noisy = torch.from_numpy(np.float32(self.noisy_images[tar_index]))
